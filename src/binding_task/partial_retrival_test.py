...
class PartialRetrivalTest(TestPhase):
    ...
    def run_example(self):
        self.run_test(object_path=Path(Paths.OBJECT_EXAMPLE_FORK), trial_times={}, is_example=True)
        self.run_test(object_path=Path(Paths.OBJECT_EXAMPLE_ROBOT), trial_times={}, is_example=True)
    ...
    def run(self):
        ...
        for trial_index, object_path in enumerate(self.blocks[0]):
            trial_times = {}
            subject_answer = self.run_test(object_path, trial_times=trial_times)
    ...
    def run_test(self, object_path: Path, trial_times: dict, is_example: bool = False):
        ...
        self._show_object(image_path=object_path, trial_times=trial_times, is_example=is_example)
    ...
    def _write_subject_answers(self, object_path: Path, subject_answer: dict, trial_times: dict):
        ...
        probe = subject_answer.get("probe")
        self.subject_answers[f"{StringEnums.TRIAL}_{len(self.subject_answers) + 1}"] = {
            object_name: {
                ...
                "subject_answer": subject_answer.get(probe),  # feature chosen, or None if not remembered
            },
            ...
            for obj, obj_data in trial_data.items():
                if obj not in skip_keys:
                    row = {
                        ...
                        "subject_answer": obj_data.get("subject_answer"),
                    }
    ...
    def _load_correct_objects(subject_id: str) -> list:
        ...
        only_correct = df[df['both_correct'] == True]
        return [Path(Paths.OBJECT_PATH.format(obj)) for obj in only_correct["object"].tolist()]
