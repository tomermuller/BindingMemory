...
class FunctionalLocalizer:
    ...
    def __init__(self, categories: list, win: psychopy.visual.window.Window, parallel_port: parallel.ParallelPort,
        ...
        self.all_trials = [key for category in self.category_to_features.values() for key in category.keys()] * TaskManage.NUMBER_OF_TRIALS_PER_FEATURE[self.exp]
    ...
    def _build_examples(self) -> list:
        examples = []
        for i, (category, features) in enumerate(self.category_to_features.items()):
            feature = random.choice(list(features.keys()))
            is_true = (i % 2 == 0)
            if is_true:
                word_question = feature
            else:
                word_question = random.choice([f for f in features if f != feature])
            examples.append((feature, word_question, is_true))
        return examples
    ...
    def _temp_save(self, trial: int):
        ...
        temp_save_path = f'{Paths.SAVE_TEMP_FOLDER[self.exp]}subject_{self.subject_id}/'
    ...
    def save_results(self, time):
        ...
        functional_localizer_folder = f"{Paths.SAVE_DATA_FOLDER[self.exp]}subject_{self.subject_id}/functional_localizer/"
